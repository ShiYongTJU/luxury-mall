...
import { DataSourceItem, DataSourceQueryParams, ProductListData } from '../../types/datasource'
...
function ProductListManagement() {
  ...
  const fetchItems = async (page?: number, pageSize?: number) => {
    ...
    try {
      ...
      const result = await datasourceApi.getItems('productList', {
  ...
  const handleEdit = (item: DataSourceItem) => {
    ...
    try {
      const data = JSON.parse(item.data) as ProductListData
      // 从JSON解析完整商品对象数组
  ...
  const handleSave = async () => {
    try {
      ...
      if (isEditMode && editingItem) {
        await datasourceApi.updateItem('productList', editingItem.id, {
          name: values.name,
          data: dataStr
        })
        message.success('更新成功')
      } else {
        await datasourceApi.createItem('productList', {
          name: values.name,
          dataSourceType: 'productList',
          abbreviation: getDataSourceAbbreviation('productList'),
          data: dataStr,
          sortOrder: 0,
          isEnabled: true
        })
  ...
  const handleDelete = async (item: DataSourceItem) => {
    try {
      await datasourceApi.deleteItem('productList', item.id)
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      {/* 编辑/新增模态框 */}
      <Modal
        title={isEditMode ? '编辑商品列表' : '新增商品列表'}
