...
import { DataSourceItem, DataSourceQueryParams, SeckillData } from '../../types/datasource'
...
function SeckillManagement() {
  ...
  const fetchItems = async (page?: number, pageSize?: number) => {
    ...
    try {
      ...
      const result = await datasourceApi.getItems('seckill', {
      ...
      message.error('获取秒杀列表失败：' + (error.message || '未知错误'))
      setItems([])
  ...
  const handleDelete = async (item: DataSourceItem) => {
    try {
      await datasourceApi.deleteItem('seckill', item.id)
